...
result={'minecraftVersion':VERSION,'generatedAt':datetime.now(timezone.utc).isoformat(),'totalSelectable':len(selectable),'renderable':renderable,'unsupported':counts['unsupported'],'coveragePercent':round(renderable/len(selectable)*100,2),'breakdown':dict(counts),'unsupportedBreakdown':dict(reasons.most_common()),'goldenTests':{i:plans[i] for i in golden},'method':'Every selectable definition and complete model parent chain was parsed; geometry requires non-empty vanilla elements.'}
